...
We use a large set of corresponding points\ldots
...
The idea behind the use of sampled points from the line features was to have more point correspondencies in complex regions as for example the eyes and the ears where there is a great abundancy of pixels and the algorithm isn’t likely to create a flow field which is accurate not enough to describe these regions \viscomment{Und sehr spezifische zuordnung, wenn die Augenkante versetzt ist, sieht man das sofort}, because of it’s smoothness constraint.

We have 8 contours given for every scan we want to register. These have been marked on three images of every face, see Fig section 1.3. The contours we call line features depict the eyebrows, eyes, ears and lips of a face. They are made up of a set of segments, each of which is modelled with a \textbf{B\'{e}zier curve} of a specified order. B\'{e}zier curves are often used in Computer
...
Now, establishing correspondence for a pair of ears for example is extremely difficult, even for
experts, because of their very different topology. The structure of the concha is very
complex. To define correspondence between lips, eyes and ears we move on to using contour lines of their rims. Further modelling the ratio between the different parts of these contours in would again be tedious. Therefore we simplify the assumptions for similarity to an equidistant parametrization of the line features so that the mapping $\Omega$ is approximately plausible. In effect, when a curve is sampled at N points, these N points are all at equal parametric intervals.
\begin{comment}
...
We decided to use equidistant parametrization, but bezier curves don't allow it.
...
\viscomment{gpfitting} is also statismo
